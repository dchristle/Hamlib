...
static int q900_get_freq(RIG *rig, vfo_t vfo, freq_t *freq)
{
 ...
 static int q900_get_mode(RIG *rig, vfo_t vfo, rmode_t *mode, pbwidth_t *width)
 {
    ...
    rmode_t mode_a = guohe2rmode(status.mode_a, q900_modes);
    rmode_t mode_b = guohe2rmode(status.mode_b, q900_modes);
                              ...
                              unsigned char value)
{
 ...
 static int q900_set_mode(RIG *rig, vfo_t vfo, rmode_t mode, pbwidth_t width)
 {
     ...
     if (vfo == RIG_VFO_B)
     {
         cmd[6] = rmode2guohe(other_mode, q900_modes);
         cmd[7] = i;
     }
     else
     {
         cmd[6] = i;
         cmd[7] = rmode2guohe(other_mode, q900_modes);
     }
     ...
     rig_set_cache_mode_only(rig, RIG_VFO_A,
                             guohe2rmode(reply[6], q900_modes));
     rig_set_cache_mode_only(rig, RIG_VFO_B,
                             guohe2rmode(reply[7], q900_modes));
